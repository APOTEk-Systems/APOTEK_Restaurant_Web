...
export default function BarInventory() {
  ...
  const { data: inventoryItems = [], isLoading: loadingInventory } = useQuery({
    queryKey: ['departmentInventory', 'BAR'],
    queryFn: () => InventoryService.getDepartmentInventory('BAR'),
  });

  // Fetch stock requests using React Query
  const { data: stockRequests = [], isLoading: loadingRequests } = useQuery({
    queryKey: ['stockRequests', 'BAR'],
    queryFn: () => InventoryService.getStockRequests('BAR'),
  });
  ...
  const updateInventoryMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['departmentInventory', 'BAR'] });
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <Button variant="outline" asChild>
            <a href="/stock-request?department=bar">
          ...
          <TabsContent value="items" className="mt-6">
            <div className="space-y-6">
              ...
              <Card className="glass-card">
                <CardHeader>
                  <CardTitle>Bar Inventory Items</CardTitle>
                </CardHeader>
                <CardContent>
                  {loading ? (
